...
void ShowContacts() {
  ...
  QSqlRecord rec = q.record();
  // Количество строк из базы данных
  cout << "Count: " << q.size() << endl;

  int count = 0;
  // Выбираем следующую строку
  while (q.next()) { // пока есть строки
    // Для Qt до версии 5
...
int SQLiteTest() {
  ...
  SQL_Execute("DELETE FROM contacts WHERE name");
  //  SQL_Execute("DELETE FROM contacts WHERE name = ...");
