...
// -------------------------------------------------------------------------
// SQLite - компактная встраиваемая
// реляционная база данных
// Слово «встраиваемый» означает, что
// SQLite не использует парадигму клиент-сервер,
// то есть движок SQLite не является отдельно
// работающим процессом,
// с которым взаимодействует программа,
// а предоставляет библиотеку,
...
bool TableExists(QString tableName) {
  ...
  QString queryTemplate =
    "SELECT count(*) AS tableCount "   // count(*) - просто возвращает количество записей удовлетворяющих критериям
  ...
  // %1 - первый аргумент
  // sqlite_master - служебная таблица со
  // всеми таблицами и индексами данной БД
  QSqlQuery q;
  QString sql = queryTemplate.arg(tableName);
  qDebug() << "SQL: " << sql;

  if (!q.exec(sql)) {
    // Если exec() вернул false =>
  ...
  q.next();
  int result = q.value(rec.indexOf("tableCount")).toInt();
  cout << "result " << result << endl;
  int result2 = q.value("tableCount").toInt();
...
int SQLiteTest() {
  ...
  // 1. Подключаем драйвер для работы с SQLite
  ...
  // Sqlite3
  // 2. Имя файла с SQLite базой данных
  dbase.setDatabaseName("my_db.sqlite");

  // 3. Открываем подключение к БД
  if (!dbase.open()) {
    qDebug() << "Не получилось открыть/создать "
             "файл с базой данных!";
    return -1;
  }
  ...
  SQL_Execute("DELETE FROM contacts "
              "WHERE name='Лена'");
  ...
  if (!a_query.exec("SELECT name, phone FROM"
                    ...
                    "WHERE name = 'Лена'")) {
    qDebug() << "Даже SELECT не получается, я пас.";
    return -2;
  }
